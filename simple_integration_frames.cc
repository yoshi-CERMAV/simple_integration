...
int average_frame(int frame, int f0, int f1, int n, vector<double> &q, vector<double> &avg, vector<double> &stdev, vector<int> &count){
    q.resize(n);
    avg.resize(n);
    stdev.resize(n);
    count.resize(n);
    if(detector_type==D5_WOS::WOS){
        reader->accumulate_scan_WOS(frame, f0, f1,  sample_data);
    }else{
        reader->accumulate_scan_D5(frame, f0, f1, sample_data);
    }
    double q0 = poni->qmin();
    double q1 = poni->qmax();
#ifdef LOG_SAMPLING
...
//        cout << i << " "<<outfile<<endl;
        ...
        average_frame(i, f0, f1, sampling, q, avg, stdev, count);
        cout << "averaged"<<endl;
        ofstream fo(fname);
